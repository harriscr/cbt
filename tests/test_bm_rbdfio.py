...
class TestBenchmarkrbdfio(unittest.TestCase):
    ...
    def test_valid__base_archive_directory(self):
        ...
        self.assertEqual(self.bl_json['rbdfio']['_base_archive_directory'], b.__dict__['_base_archive_directory'])

    def test_valid__create_report(self):
        """ Basic sanity attribute identity _create_report check"""
        b = benchmarkfactory.get_object(self.archive_dir,
                                            self.cluster, 'rbdfio', self.iteration)
        self.assertEqual(self.bl_json['rbdfio']['_create_report'], b.__dict__['_create_report'])
